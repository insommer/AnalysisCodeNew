...
data_folder = [
    fr'{camera}/Beam focus step 30 to 15 percent acqRate 10'
    
    ]

acquisitionRate_ms = 20
...
pattern = os.path.join(dataPath[0], "*_[0-9][0-9][0-9][0-9].tiff")
...
ax[1].plot(df['Time'], df['Xcenter']/1000, label='Xcenter')
ax[1].plot(df['Time'], df['Ycenter']/1000, label='Ycenter')
ax[1].set_title('Center')
ax[1].set_xlabel('Time (s)')
ax[1].set_ylabel('mm')
ax[1].legend()

ax[2].plot(df['Time'], df['Xamp'], label='Xamp')
ax[2].plot(df['Time'], df['Yamp'], label='Yamp')
